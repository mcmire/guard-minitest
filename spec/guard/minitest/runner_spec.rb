    ...
    describe 'drb' do
      describe 'when using test_helper' do
        it 'should run with drb' do
          ...
          runner.expects(:system).with(
            "GUARD_NOTIFY=false testdrb -r #{File.expand_path('.')}/lib/guard/minitest/runners/default_runner.rb test/test_helper.rb ./test/test_minitest.rb"
          )
        ...
        it 'should run with drb and notification enable' do
          ...
          File.expects(:exist?).with('test/spec_helper.rb').returns(false)
          runner.expects(:system).with(
            "GUARD_NOTIFY=true testdrb -r #{File.expand_path('.')}/lib/guard/minitest/runners/default_runner.rb test/test_helper.rb ./test/test_minitest.rb"
          )
          runner.run(['test/test_minitest.rb'], :drb => true)
        end
      end
      ...
      describe 'when using spec_helper' do
        it 'should run with drb' do
          ...
          runner.expects(:system).with(
            "GUARD_NOTIFY=false testdrb -r #{File.expand_path('.')}/lib/guard/minitest/runners/default_runner.rb spec/spec_helper.rb ./test/test_minitest.rb"
          )
          runner.run(['test/test_minitest.rb'], :drb => true)
        end
        ...
        it 'should run with drb and notification disable' do
          ...
          File.expects(:exist?).with('spec/spec_helper.rb').returns(true)
          runner.expects(:system).with(
            "GUARD_NOTIFY=true testdrb -r #{File.expand_path('.')}/lib/guard/minitest/runners/default_runner.rb spec/spec_helper.rb ./test/test_minitest.rb"
          )
          runner.run(['test/test_minitest.rb'], :drb => true)
        end
      end
